...
public class WmDownloadListener implements DownloadListener {
    ...
    new AlertDialog.Builder(activity)
      ...
      .setPositiveButton(R.string.alertdialog_downloadlistener_label_button_positive, new DialogInterface.OnClickListener() {
        public void onClick(DialogInterface dialog, int id) {
          ...
          if (is_data) {
            final String _contentDisposition = TextUtils.isEmpty(contentDisposition) ? "attachment;filename=\"data\"" : contentDisposition;
            final String fileName = URLUtil.guessFileName(null, _contentDisposition, mimetype);
            Thread thread = new Thread() {
              @Override
              public void run() {
  ...
  private void downloadHttpUri(String url, String fileName) {
    try {
      ...
      final SaveFileHelper.Download download = new SaveFileHelper.Download(cacheUUID, mimeType);
      final String targetFileName = fileName;
      activity.runOnUiThread(new Runnable() {
        @Override
        public void run() {
          SaveFileHelper.showFilePicker(activity, download, targetFileName);
        }
      });
    }
    catch(Exception e) {
